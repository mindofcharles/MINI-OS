...
| Private Ethernet framing | `net_ethernet_decode`, `net_ethernet_begin_tx`, `net_ethernet_send` | deterministic host tests and target compilation | internal to the modern-C network library; ARP and IPv4 classification only; no public polling or protocol processing yet |
